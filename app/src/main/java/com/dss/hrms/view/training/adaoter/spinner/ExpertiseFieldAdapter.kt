...
class ExpertiseFieldAdapter {

    fun setExpertiseFieldSpinner(
        spinner: Spinner,
        context: Context?,
        dataList: List<ExpertiseField?>?,
        id: Int?,
        ...
        dataList?.let {
            while (i < it.size) {
                if (preparence?.getLanguage().equals("en"))
                    list.add(dataList.get(i)?.expertise_name)
                else
                    list.add(dataList.get(i)?.expertise_name_bn)
                if (it.get(i)?.id == id) {
