...
class EmployeePersonalInfoViewModel_honourAward :ViewModel(){
    var resultLiveData : MutableLiveData<Any>?=null
    fun honoursAward(context: Context): MutableLiveData<Any> {
        val sharedPref = SharedPref(context)
        if (resultLiveData == null) {
            resultLiveData = MutableLiveData()
        }
        val call: Call<HonorAwardRes> = RetrofitClient.getInstance().getApi().HonorAwardRes_(
            "application/json","application/json",sharedPref.access_token!!,sharedPref.uEmployeeId!!
        )
        call.enqueue(object : Callback<HonorAwardRes> {
            override fun onResponse(call: Call<HonorAwardRes>, response: Response<HonorAwardRes>) {
            ...
            override fun onFailure(call: Call<HonorAwardRes>, t: Throwable) {
                resultLiveData!!.postValue(null)
